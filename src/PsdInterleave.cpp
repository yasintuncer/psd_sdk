...
namespace
{
	...
	template <>
	__m128i InterleaveLo<1>(__m128i a, __m128i b) { return _mm_unpacklo_epi8(a, b); }
	template <>
	__m128i InterleaveLo<2>(__m128i a, __m128i b) { return _mm_unpacklo_epi16(a, b); }
	template <>
	__m128i InterleaveLo<4>(__m128i a, __m128i b) { return _mm_unpacklo_epi32(a, b); }
	template <>
	__m128i InterleaveLo<8>(__m128i a, __m128i b) { return _mm_unpacklo_epi64(a, b); }
	...
	template <>
	__m128i InterleaveHi<1>(__m128i a, __m128i b) { return _mm_unpackhi_epi8(a, b); }
	template <>
	__m128i InterleaveHi<2>(__m128i a, __m128i b) { return _mm_unpackhi_epi16(a, b); }
	template <>
	__m128i InterleaveHi<4>(__m128i a, __m128i b) { return _mm_unpackhi_epi32(a, b); }
	template <>
	__m128i InterleaveHi<8>(__m128i a, __m128i b) { return _mm_unpackhi_epi64(a, b); }
}
#endif
...
namespace imageUtil
{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	template <typename T>
	unsigned int InterleaveRGBBlocks(const T *PSD_RESTRICT srcR, const T *PSD_RESTRICT srcG, const T *PSD_RESTRICT srcB, T alpha, T *PSD_RESTRICT dest, unsigned int width, unsigned int height, unsigned int blockSize)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	template <typename T>
	unsigned int InterleaveRGBABlocks(const T *PSD_RESTRICT srcR, const T *PSD_RESTRICT srcG, const T *PSD_RESTRICT srcB, const T *PSD_RESTRICT srcA, T *PSD_RESTRICT dest, unsigned int width, unsigned int height, unsigned int blockSize)
	{
		const unsigned int pixelCount = width * height;
		const unsigned int blockCount = pixelCount / blockSize;
	...
	// ---------------------------------------------------------------------------------------------------------------------
	template <typename T>
	void CopyRemainingRGBPixels(const T *PSD_RESTRICT srcR, const T *PSD_RESTRICT srcG, const T *PSD_RESTRICT srcB, T alpha, T *PSD_RESTRICT dest, unsigned int count)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	template <typename T>
	void CopyRemainingRGBAPixels(const T *PSD_RESTRICT srcR, const T *PSD_RESTRICT srcG, const T *PSD_RESTRICT srcB, const T *PSD_RESTRICT srcA, T *PSD_RESTRICT dest, unsigned int count)
	{
		for (unsigned int i = 0; i < count; ++i)
		{
			const T r = srcR[i];
	...
	void InterleaveRGB(const T *PSD_RESTRICT srcR, const T *PSD_RESTRICT srcG, const T *PSD_RESTRICT srcB, T alpha, T *PSD_RESTRICT dest, unsigned int width, unsigned int height, unsigned int blockSize)
	{
		...
		const unsigned int blockCount = InterleaveRGBBlocks(srcR, srcG, srcB, alpha, dest, width, height, blockSize);
		const unsigned int remaining = width * height - blockCount * blockSize;
		CopyRemainingRGBPixels(srcR + blockCount * blockSize, srcG + blockCount * blockSize, srcB + blockCount * blockSize, alpha, dest + blockCount * blockSize * 4u, remaining);
	}
	...
	{
		// do blocks first, and then copy remaining pixels
		const unsigned int blockCount = InterleaveRGBABlocks(srcR, srcG, srcB, srcA, dest, width, height, blockSize);
		const unsigned int remaining = width * height - blockCount * blockSize;
		CopyRemainingRGBAPixels(srcR + blockCount * blockSize, srcG + blockCount * blockSize, srcB + blockCount * blockSize, srcA + blockCount * blockSize, dest + blockCount * blockSize * 4u, remaining);
	}
	...
	void InterleaveRGB(const T *PSD_RESTRICT srcR, const T *PSD_RESTRICT srcG, const T *PSD_RESTRICT srcB, T alpha, T *PSD_RESTRICT dest, unsigned int width, unsigned int height, unsigned int /* blockSize */)
	{
		...
		CopyRemainingRGBPixels(srcR, srcG, srcB, alpha, dest, count);
	}
	...
	void InterleaveRGBA(const T *PSD_RESTRICT srcR, const T *PSD_RESTRICT srcG, const T *PSD_RESTRICT srcB, const T *PSD_RESTRICT srcA, T *PSD_RESTRICT dest, unsigned int width, unsigned int height, unsigned int /* blockSize */)
	{
		// copy pixels
		unsigned int count = width * height;
		CopyRemainingRGBAPixels(srcR, srcG, srcB, srcA, dest, count);
	}
